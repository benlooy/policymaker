...
def create_policy(policy_row: pd.Series, rules: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    policy_name = policy_row.get("policy_display_name", policy_row.get("Application", "default-policy"))  # Use exact capitalization from input
    ...
    return {
        "nsxt_policy_security_policy": {
            f"{policy_name}": {
                "nsx_id": "",
                "policy_display_name": policy_name,
...
def main():
    ...
    try:
        ...
        # Create the combined policies
        combined_policy = create_combined_policies(policy_rules_pairs)
        
        # Extract the application field value
        application_name = policy_rules_pairs[0][0].iloc[0].get("policy_display_name", "default-application")
        print(f"Application name: {application_name}")  # Debugging information
        
        # Create a new folder named after the application field value
        output_dir = os.path.join("output_applications", application_name)
        print(f"Creating output directory: {output_dir}")  # Debugging information
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        
        # Generate output filename using policy_display_name field with "_policies" appended
        output_file = os.path.join(output_dir, f"{application_name}_policies.tf.json")
        print(f"Output file path: {output_file}")  # Debugging information
        
        # Check if file exists and confirm overwrite
        if check_and_confirm_overwrite(output_file):
            # Write the JSON file
            with open(output_file, 'w') as f:
                json.dump(combined_policy, f, indent=2)
