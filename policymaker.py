...
def construct_variable_reference(type_str: str, name: str) -> str:
    """
    Constructs variable reference based on type and name
    """
    if type_str == "service":
        ...
        if name.endswith('_path'):
            return f"${{var.infra_groups_path.{name}}}"
        return name
...
def validate_category(category: str) -> str:
    ...
    Returns the category if valid, raises an error if not.
    """
    valid_categories = ["application", "infrastructure", "environment"]
    category = category.lower()  # Convert to lowercase for comparison
    if category not in valid_categories:
        raise ValueError(f"Category must be one of: {', '.join(valid_categories)}. Got: {category}")
    return category
...
def create_policy(policy_row: pd.Series, rules: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    policy_name = policy_row.get("Application", "default-policy").lower()
        ...
        "resource": [{
            "nsxt_policy_security_policy": {
                f"{policy_name}": {
                    ...
                    "sequence_number": int(policy_row.get("sequence_number", 0)),
                    "rule": rules
                }
            }
        }]
...
def create_rule(rule_row: pd.Series) -> Dict[str, Any]:
    ...
    with open('logs/debug.log', 'a') as log_file:
        ...
        log(f"Raw destinations_excluded value: {rule_row.get('destinations_excluded (Negate)')}")
        log(f"Type of destinations_excluded: {type(rule_row.get('destinations_excluded (Negate)'))}")
        ...
        rule = {
            ...
            "display_name": rule_row.get("display_name", ""),
        ...
        log(f"Final destinations_excluded value: {rule['destinations_excluded']}")
        
        # Get destination groups
        destination_groups = rule_row.get("destination_groups")
        if pd.notna(destination_groups) and destination_groups != "" and destination_groups != "any":
            groups = destination_groups.split(',')
            rule["destination_groups"] = [
                construct_variable_reference("group", group.strip())
                for group in groups
            ]
        
        # Get services
        services = rule_row.get("services")
        if pd.notna(services) and services != "" and services != "any":
        ...
        # Get source groups
        source_groups = rule_row.get("source_groups")
        if pd.notna(source_groups) and source_groups != "" and source_groups != "any":
            groups = source_groups.split(',')
            rule["source_groups"] = [
                construct_variable_reference("group", group.strip())
                for group in groups
            ]
...
def main():
    ...
    try:
        ...
        # Create rules list
        rules = [create_rule(row) for _, row in rules_df.iterrows()]
        
        # Create the complete policy
        policy = create_policy(policy_df.iloc[0], rules)
        
        # Generate output filename
        app_name = policy_df.iloc[0].get("Application", "default-policy").lower()
        output_file = os.path.join("output", f"{app_name}.tf.json")
